...
const publishVideo = asyncHandle(async (req, res) => {
    ...
    const publishedThumbnail = await uplodeOnCloudinary(thumbnailocalPath);
    const publishedVideo = await uplodeOnCloudinary(videoLocalPath);
...
const updateVideo = asyncHandle(async(req,res)=>{
    ...
    const thumbnail = await uplodeOnCloudinary(thumbnaiLocalPath);
...
const toggleIsPublished = asyncHandle(async(req,res)=>{
    const {videoId} = req.params;
